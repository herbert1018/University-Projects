import sys
import os
from EfficientNetB0 import EfficientNetB0Model, train_model, get_dataloader, save_model, load_model
...
if __name__ == '__main__':
    ...
    model_save_path = os.path.join(model_dir, "EfficientNetB0_model.pth")
...
def train():
    ...
    # 檢查是否已存在模型
    if os.path.exists(model_save_path):
        show_temp_message("已有EfficientNetB0模型存在！", "red")
        return
...
def test():
    ...
    global model
    try:
        model_path = os.path.join(model_dir, "EfficientNetB0_model.pth")
...
def main():
    ...
    global root
    root = tk.Tk()
    root.title("EfficientNetB0 訓練介面")
