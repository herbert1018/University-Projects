from AlexNet import AlexNet, train_model, test_model, get_dataloader, save_model, load_model
...
if __name__ == '__main__':
    ...
    model_save_path = os.path.join(model_dir, "alexnet_model.pth")
    ...
    try:
        model = AlexNet(num_classes=5, input_size=(256,256)).to(device)
    ...
    try:
        ...
        print(f"Error defining loss function or optimizer: {e}")
    ...
    # 在檔案開頭的全域變數區域添加
    last_message_timer = None  # 用於追蹤最後一個訊息的計時器ID
...
def create_gui_elements():
    ...
    # 創建左側按鈕區域
    left_frame = tk.Frame(root, bg="#e0e0e0", width=350)  # 加寬並改變背景色
    left_frame.pack(side=tk.LEFT, padx=20, fill=tk.Y)
    left_frame.pack_propagate(False)

    # 創建按鈕容器
    button_container = tk.Frame(left_frame, bg="#e0e0e0")  # 使用相同背景色
    ...
    # 結果顯示標籤
    global result_label
    result_label = tk.Label(button_container, text="", bg="#e0e0e0",  # 使用相同背景色
...
def train():
    ...
    # 檢查是否已存在模型
    if os.path.exists(model_save_path):
        show_temp_message("已有AlexNet模型存在！", "red")
        return
...
def test():
    ...
    global model
    try:
        model_path = os.path.join(model_dir, "alexnet_model.pth")
...
def main():
    ...
    global root
    root = tk.Tk()
    root.title("AlexNet 訓練介面")
    root.geometry("1000x1000")  # 加寬視窗以容納右側內容
