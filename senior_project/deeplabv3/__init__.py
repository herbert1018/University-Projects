...
import os
import time
from DeepLab_v3 import DeepLabV3Plus, SegmentationDataset, train_model, get_dataloader, save_model
...
if __name__ == '__main__':
    ...
    model_save_path = os.path.join(model_dir, "deeplab_model.pth")
    ...
    # 設定超參數
    backbone_model='resnet' # 可選 'resnet' 或 'xception'
    batch_size = 8
    learning_rate = 5e-4
    weight_decay = 5e-4
    num_epochs = 30
    patience_epochs = 10 # Early Stopping 的耐心值
    ...
    # 初始化模型和優化器
    try:
        model = DeepLabV3Plus(num_classes=2, backbone_type=backbone_model).to(device)
...
def create_gui_elements():
    ...
    global line_train_loss, line_val_loss, line_val_acc, line_val_dice
    ...
    ax_loss.set_xlim(0, num_epochs+1)  # 設定 x 軸範圍
    ...
    # Metric 曲線圖 (val acc: 綠線, val dice: 紫線)
    ...
    line_val_dice, = ax_metric.plot([], [], 'm--', label='Val Dice')
    ax_metric.legend()
...
def update_training_progress(epoch, total_epochs, batch, total_batches,
                              train_loss=None, val_loss=None, val_acc=None, val_dice=None):
    ...
    if batch == total_batches:
        ...
        # 自動調整 y 軸範圍
        ax_loss.relim()             # 計算新的資料範圍
        ax_loss.autoscale_view()    # 套用新的範圍
        ax_loss.set_xlim(0, total_epochs+1)  # 固定 x 軸
        canvas_loss.draw()
        ...
        if val_dice_history:
            ...
            line_val_dice.set_data(x_dice, val_dice_history)
...
def train():
    ...
    if os.path.exists(model_save_path):
        show_temp_message("已有DeepLabV3Plus模型存在！", "red")
        return
    ...
    try:
        ...
        model, best_dice, minutes, seconds = train_model(
            model, 
            train_loader,
            criterion, 
            optimizer, 
            ...
            patience=patience_epochs,  # Early Stopping
            device=device
        )
        save_model(model, model_save_path)
        
        completion_text = f"訓練完成！花費時間: {minutes}分{seconds}秒，最佳Dice: {best_dice:.4f}"
...
def load_compatible_state_dict(model, ckpt_path):
    if not os.path.exists(ckpt_path):
        raise FileNotFoundError(f"模型檔案不存在: {ckpt_path}")

    state = torch.load(ckpt_path, map_location="cpu", weights_only=True)
    sd = state.get("state_dict", state.get("model", state))  # 兼容常見格式
    ...
    for k, v in sd.items():
        ...
        # 映射舊鍵到新鍵
        if k.startswith("low_level_conv.0."):         # Conv 1x1
            k = k.replace("low_level_conv.0.", "decoder.conv1.")
        elif k.startswith("low_level_conv.1."):       # BatchNorm
            k = k.replace("low_level_conv.1.", "decoder.bn1.")
...
def test():
    ...
    try:
        model = DeepLabV3Plus(num_classes=2, backbone_type=backbone_model).to(device)
        model = load_compatible_state_dict(model, model_save_path)  # 使用兼容函數
        model.eval()
        with torch.no_grad():
