import sys
import os
from RepVGG import RepVGGModel, train_model, get_dataloader, save_model, load_model
...
if __name__ == '__main__':
    ...
    model_save_path = os.path.join(model_dir, "repvgg_model.pth")
    ...
    try:
        # 五類分類
        model = RepVGGModel(num_classes=5, deploy=False).to(device)
        state_dict = torch.load("repVGG/RepVGG-A0-train.pth", map_location=device, weights_only=True)
        # 過濾掉分類層（linear層），保留特徵提取部分
        new_state_dict = OrderedDict()
        
        for k, v in state_dict.items():
            if "linear" not in k:  # 保留所有非分類層的權重
                new_state_dict[k] = v
            else:
                print(f"Skipping layer: {k} with shape {v.shape}")
...
def train():
    ...
    # 檢查是否已存在模型
    if os.path.exists(model_save_path):
        show_temp_message("已有RepVGG模型存在！", "red")
        return
...
def test():
    ...
    global model
    try:
        model_path = os.path.join(model_dir, "repvgg_model.pth")
